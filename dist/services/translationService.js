...
class TranslationService {
    ...
    async createLanguage(code, name, is_active = true) {
        const query = `
      INSERT INTO languages (code, name, is_active)
      VALUES ($1, $2, $3)
      RETURNING *
    `;
        const result = await database_1.default.query(query, [code, name, is_active]);
        return result.rows[0];
    }
    async updateLanguage(id, name, is_active) {
        const query = `
      UPDATE languages 
      ...
      RETURNING *
    `;
        const result = await database_1.default.query(query, [name, is_active, id]);
    ...
    async createTranslationKey(keyData) {
      ...
      RETURNING *
    `;
        const result = await database_1.default.query(query, [key_name, description, category]);
        return result.rows[0];
    }
    async updateTranslationKey(id, keyData) {
        const updateFields = [];
        const values = [];
        let paramIndex = 1;
    ...
    async getTranslationsWithDetails() {
        const query = `
      SELECT 
        tk.id,
        tk.key_name,
    ...
    `;
        const result = await database_1.default.query(query, [languageCode]);
        const translations = {};
    ...
    async updateTranslation(translationKeyId, languageCode, translationData) {
    ...
    async deleteTranslation(translationKeyId, languageCode) {
        const query = `
      DELETE FROM translations 
      WHERE translation_key_id = $1 
      AND language_id = (SELECT id FROM languages WHERE code = $2)
    `;
        const result = await database_1.default.query(query, [translationKeyId, languageCode]);
        return result.rowCount !== null && result.rowCount > 0;
    }
    ...
    async importTranslations(importData) {
        const client = await database_1.default.connect();
        let imported = 0;
        const errors = [];
        try {
